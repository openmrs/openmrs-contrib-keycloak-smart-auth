...
public class ExternalAppAuthenticator implements Authenticator {
	...
	public static final String DEFAULT_EXTERNAL_APP_URL = "http://localhost:8080/openmrs/smartonfhir/findPatient.page?app=smart.search&token={TOKEN}";
	...
	public void authenticate(AuthenticationFlowContext context) {
		...
		if (context.getAuthenticatorConfig() != null) {
			externalApplicationUrl = context.getAuthenticatorConfig().getConfig()
					.get(ExternalAppAuthenticatorFactory.CONFIG_EXTERNAL_APP_URL);
			applicationId = context.getAuthenticatorConfig().getConfig()
					.get(ExternalAppAuthenticatorFactory.CONFIG_APPLICATION_ID);
		}
		...
		try {
			Response challenge = Response
					.status(Status.FOUND)
					.header("Location",
							externalApplicationUrl.replace("{TOKEN}", URLEncoder.encode(submitActionTokenUrl, "UTF-8")))
					.build();
	...
	public void action(AuthenticationFlowContext context) {
		final AuthenticationSessionModel authSession = context.getAuthenticationSession();
		if (!Objects.equals(authSession
				.getAuthNote(ExternalApplicationNotificationActionTokenHandler.INITIATED_BY_ACTION_TOKEN_EXT_APP), "true")) {
		...
		try {
			...
			appToken.getOtherClaims()
					.forEach((key, value) -> user
							.setAttribute(appId + "." + key, Collections.singletonList(String.valueOf(value))));
		}
